...
- Comprendre la différence fondamentale entre SLI, SLO et SLA
- Savoir choisir les bons SLIs pour un service donne
- Définir des SLOs realistes et mesurables
...
Sans ces concepts, les équipes tombent dans deux pieges :
1. **Sur-ingenierie** : viser 100% de disponibilité (impossible et ruineux)
2. **Sous-ingenierie** : ne pas savoir quand la qualite se degrade

Le framework SLI/SLO/SLA, popularise par le livre **Google SRE**, fournit un langage commun entre équipes produit, développement et operations.
...
Un **SLI** (Service Level Indicator) est une **mesure quantitative** d'un aspect du service tel que percu par l'utilisateur. C'est la réponse à la question : **"Que mesurons-nous ?"**
...
Proportion de requêtes qui reussissent :
...
Proportion de requêtes plus rapides qu'un seuil :
...
Proportion du debit traite par rapport à la capacité attendue :
...
Proportion de réponses qui retournent le bon résultat :
...
Un **SLO** (Service Level Objective) est un **objectif cible** pour un SLI. C'est la réponse à la question : **"Quel niveau visons-nous ?"**
...
| Service | SLI | SLO |
|---------|-----|-----|
| API REST | Disponibilité | 99.9% sur 30 jours |
...
Ne visez jamais 100%. Un SLO de 100% signifie que vous ne pouvez **jamais** déployer, car tout changement comporte un risque. Même Google ne vise que 99.99% pour ses services les plus critiques.
:::

### Rolling Window (fenêtre glissante)

Plutot qu'un mois calendaire, on utilise généralement une **fenêtre glissante de 30 jours**. Avantage : pas de "remise a zero" le 1er du mois qui encouragerait des prises de risque.
...
Un **SLA** (Service Level Agreement) est un **contrat** entre un fournisseur et un client. Il définit les **consequences** (financieres ou autres) si les SLOs ne sont pas respectes.
...
## Error Budget — Le concept clé

### Definition

L'**error budget** est la quantite d'erreurs que vous **pouvez vous permettre** avant de violer votre SLO. C'est la différence entre la perfection (100%) et votre SLO.
...
Pensez a l'error budget comme un portefeuille mensuel. Chaque incident "depense" une partie du budget. Quand le portefeuille est vide, vous devez arreter de depenser (geler les deployments, investir dans la fiabilité).
...
Une **error budget policy** définit les actions a prendre selon le niveau de consommation du budget :
...
| 75-100% | Gel des features | Seuls les correctifs et ameliorations de fiabilité |
| >100% | Sprint fiabilité | Toute l'équipe se concentre sur la stabilite |
...
::: warning Attention
En architecture distribuee, les SLOs individuels se **multiplient**. 3 services a 99.9% chacun donnent un SLO composite d'environ 99.7%. C'est pourquoi les micro-services critiques doivent avoir des SLOs très eleves.
:::
...
1. **Commencez simple** : un SLO de disponibilité et un de latence suffisent pour debuter
...
4. **Documentez les decisions** : pourquoi ce seuil ? pourquoi cette fenêtre ?
...
::: tip A retenir
- **SLI** = ce que vous mesurez (ratio d'événements bons / total)
...
- L'error budget est un **outil de negociation** entre vitesse et fiabilité
...
- [Lab 10 — Définir des SLOs pour la demo-app](/labs/lab-10-definir-slos/README)
...
1. **Screencast** : [screencast 10 sli slo sla](../screencasts/screencast-10-sli-slo-sla.md)
2. **Lab** : [lab-10-définir-slos](../labs/lab-10-definir-slos/README)
3. **Visualisation** : [SLO Error Budget](../visualizations/slo-error-budget.html)
4. **Quiz** : [quiz 10 sli slo sla](../quizzes/quiz-10-sli-slo-sla.html)
:::
