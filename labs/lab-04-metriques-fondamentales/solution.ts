// =============================================================================
// Lab 04 — Premiers pas metriques (SOLUTION)
// =============================================================================
...
class Histogram {
  ...
  getBuckets(): Map<number, number> {
    ...
    for (const bound of this.bucketBounds) {
      const count = this.values.filter((v) => v <= bound).length;
      buckets.set(bound, count);
    }

    // +Inf bucket contains all observations
    buckets.set(Infinity, this.values.length);
...
function countRequestsByStatus(
  ...
  for (const req of requests) {
    if (!counters.has(req.status)) {
      counters.set(req.status, new Counter(`http_requests_${req.status}`));
    }
    counters.get(req.status)!.inc();
  }

  const result = new Map<number, number>();
  for (const [status, counter] of counters) {
    result.set(status, counter.get());
  }
  return result;
}
...
function analyzeLatencies(
  ...
  const histogram = new Histogram('request_duration_ms', [10, 25, 50, 100, 250, 500, 1000]);
...
async function main() {
  ...
  await test('Ex3 — Histogram buckets', () => {
    ...
    assertEqual(buckets.get(0.1), 1);
    assertEqual(buckets.get(0.5), 2);
    assertEqual(buckets.get(1.0), 3);
    assertEqual(buckets.get(Infinity), 4);
  });
